...
use crate::server::{
    ...
    // routes::{redirect::RedirectOr},
    services::factures_toasty::select_all,
    //     services::{
    //         clients,
...
async fn list_factures(State(mut db): State<Db>) -> Result<Markup, AppError> {
    let factures_data = select_all(&mut db).await?;
...
pub fn facture_router() -> Router<Db> {
    Router::new()
